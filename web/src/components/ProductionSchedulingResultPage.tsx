        ...
        data.forEach(item => {
            if (item.timeslots) {
                item.timeslots.forEach((timeslot: Timeslot) => {
                    if (timeslot.startTime) dateSet.add(timeslot.startTime.substring(0, 10));
                    if (timeslot.endTime) dateSet.add(timeslot.endTime.substring(0, 10));
                });
            }
        });
        ...
        return data.map(item => {
            ...
            dates.forEach(date => {
                row[date] = item.timeslots?.filter((ts: Timeslot) => {
                    ...
                    const startDate = ts.startTime.substring(0, 10);
                    const endDate = ts.endTime.substring(0, 10);
